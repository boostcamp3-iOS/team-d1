        ...
        manager.signIn(email: "t1@naver.com", password: "123456") { (result) in
            ...
            case .success(let data):
                ...
                for i in 0...59 {
                    ...
                    manager.uploadArtwork(image: image, scale: 0.01, path: "artworks", fileName: "test\(i)", completion: { (result) in
                        switch result {
                        case .failure(let error):
                            print(error)
                            return
